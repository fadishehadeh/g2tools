...
// Auto serial
$stmt = db()->prepare("SELECT COUNT(*) FROM form_submissions WHERE form_type='credit_note'");
$stmt->execute();
$serial = $pfx . '-CN-' . str_pad((int)$stmt->fetchColumn() + 1, 5, '0', STR_PAD_LEFT);

// Amount in words
function num_words_cn(float $n, string $cur): string {
...
$hdr=[['CN No:',$serial],['To',$to_name],['Attention',$attention],['Date',$dn_date],['Currency',$currency]];
...
$pdf->SetX($lm);
$pdf->Cell(0,7,'Kindly Note That We Have Credited Your Account With The Following:',0,1);
$pdf->Ln(4);
...
$pdf->SetX($lm); $pdf->Cell(0,7,num_words_cn($total,$currency).' only.',0,1);
$pdf->Ln(6);
...
$pdf->SetX($lm); $pdf->Cell(0,5,'ccc/cn',0,1);

// Save
$uid=$_SESSION['g2_user']['id']; $ts=date('Ymd_His');
$filename='cn_'.$uid.'_'.$ts.'.pdf';
...
if ($fin_emails && file_exists($filepath)) {
    $subj = "New Credit Note — To: {$to_name} [#{$sub_id}]";
    $html = mail_template("New Credit Note", "
        <p>A new Credit Note has been submitted and requires your approval.</p>
        <div class='info-box'><strong>Reference</strong> #$sub_id</div>
        <div class='info-box'><strong>To</strong> " . htmlspecialchars($to_name) . "</div>
        <div class='info-box'><strong>Total</strong> " . htmlspecialchars($currency) . " " . number_format($total, 2) . "</div>
